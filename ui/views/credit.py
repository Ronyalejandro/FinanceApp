...
class CreditView(ctk.CTkFrame):
    ...
    def _setup_ui(self):
        ctk.CTkLabel(self, text="Credit Line", font=FONT_TITLE_MAIN, text_color=COLOR_TEXT_WHITE).pack(anchor="w", pady=(0, 40))
    ...
    def _refresh_data(self):
        ...
        ctk.CTkLabel(credit_card, text=f"Total Limit: ${lim:.2f}", font=FONT_SUBTITLE).pack(pady=(20, 10))
        ...
        ctk.CTkLabel(credit_card, text=f"Used: ${used:.2f}  |  Available: ${disp:.2f}", font=FONT_SUBTITLE, text_color=COLOR_TEXT_WHITE).pack(pady=(10, 20))
        ...
        ctk.CTkEntry(actions_frame, textvariable=self.new_limit_var, placeholder_text="New Limit Amount").grid(row=0, column=0, padx=20, pady=20)
        ctk.CTkButton(actions_frame, text="Update Limit", command=self.update_limit, fg_color=COLOR_ACCENT_BLUE).grid(row=0, column=1, padx=20)
        ...
        ctk.CTkEntry(pay_frame, textvariable=self.pay_credit_var, placeholder_text="Payment Amount").pack(side="left", padx=20, pady=20)
        ctk.CTkButton(pay_frame, text="Make Payment", fg_color=COLOR_ACCENT_GREEN, command=self.pay_credit).pack(side="left", padx=20)
