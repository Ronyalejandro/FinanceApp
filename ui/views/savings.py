...
class SavingsView(ctk.CTkFrame):
    ...
    def _setup_ui(self):
        ...
        ctk.CTkEntry(add_frame, textvariable=self.goal_name, placeholder_text="Nombre de la Meta").pack(side="left", padx=20, expand=True, fill="x", pady=20)
        ctk.CTkEntry(add_frame, textvariable=self.goal_target, placeholder_text="Monto Objetivo").pack(side="left", padx=20)
        ctk.CTkButton(add_frame, text="Crear Meta", command=self.create_goal, fg_color=COLOR_ACCENT_BLUE).pack(side="left", padx=20)
    ...
    def refresh_goals(self):
        ...
        for g in self.db.get_savings_goals():
            ...
            ctk.CTkButton(card, text="Depositar $50", height=32, corner_radius=8, command=lambda i=gid: self.add_savings(i, 50),
