...
class Sidebar(ctk.CTkFrame):
    def __init__(self, parent, command_callback):
        ...
        self.buttons = {}
        self.indicators = {}
        self.active_view = "dashboard" # Default active
    ...
    def _create_nav_button(self, text, view_name, row):
        # Indicator strip (hidden by default)
        ...
        btn.grid(row=row, column=0, padx=(10, 0), pady=2, sticky="ew") # Pad left to make room for indicator visually or just overlap
    ...
    def _select_button(self, view_name):
        ...
        if view_name in self.buttons:
            # Illuminated text (Cyan) + Indicator
            self.buttons[view_name].configure(text_color=theme_color(COLOR_ACCENT_GREEN), fg_color=theme_color(COLOR_BTN_ACTIVE)) # Slightly lighter bg
